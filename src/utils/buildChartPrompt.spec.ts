...
describe('buildChartPrompt', () => {
  ...
  it('should generate an unusual prompt with the indicator label', () => {
    ...
    expect(prompt).toBe(
      'How unusual is the above ground biomass value compared to the typology distribution?',
    );
  });
  ...
  it('should generate a drivers prompt with the indicator label', () => {
    ...
    expect(prompt).toBe(
      'What ecological factors might explain why this location has a high canopy height value?',
    );
  });
});
